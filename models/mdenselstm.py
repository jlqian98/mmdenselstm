...
class LSTM(nn.Module):
    def __init__(self, in_dim=63, hidden_dim=128, C=12, n_layer=2):
...
class MDenseLSTM(nn.Module):
    def __init__(self, input_channel=1, drop_rate=0.1):
        super(MDenseLSTM, self).__init__()
        kl_full = [(12, 4), (12, 4), (12, 4), (12, 4), (12, 4), (12, 4), (12, 4),]
        self.fullNet = _MDenseLSTM_STEM(input_channel=input_channel, first_channel=32, first_kernel=(4, 3), scale=3, kl=kl_full, drop_rate=drop_rate, )
...
class _MDenseLSTM_STEM(nn.Module):
    ...
    def forward(self, input):
        ...
        ## upsample way
        output = self.dense_layers[self.scale](output)  # [M, 1, N, T]
